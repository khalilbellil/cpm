...
//#region FUNCTIONS
    const saveAjax = (table, uid, one, one_val) => {
        ...
        fetch(encodeURI(`http://${apiAdress}/update/one?table=${table}&uid=${uid}&one=${one}&one_val=${one_val}`))
        .then(() => props.loadingSpinner(false))
        .catch(err => alert(err))
    }
    const getProjectData = (uid_project) => {
        fetch(`http://${apiAdress}/projects/get?uid=${uid_project}`)
    ...
    const getSubServicesData = (uid_service) => {
        if(uid_service !== undefined){
            fetch(`http://${apiAdress}/subservices?uid_service=${uid_service}`)
            .then(response => response.json())
            .then(response => {
    ...
    const getAddress = (uid_address) => {
        if (uid_address)
            fetch(`http://${apiAdress}/address?uid=${uid_address}`)
            .then(response => response.json())
            .then(response => {
                response.data[0] = {...response.data[0], 
    ...
    const getCallBackLater = (uid_project) => {
        fetch(`http://${apiAdress}/callbacklater/get?uid_project=${uid_project}`)
        .then(response => response.json())
        .then(response => {
    ...
    const getCancelReasons = () => {
        fetch(`http://${apiAdress}/projects/get_cancel_reasons`)
        .then(response => response.json())
        .then(response => setCancelReasonsData(response.data))
        .catch(err => alert(err))
    }
    const getSentQuestionsAnswers = (uid_project) => {
        fetch(`http://${apiAdress}/projects/get_sent_question_answer?uid_project=${uid_project}`)
        .then(response => response.json())
        .then(response => {
    ...
    const saveAjaxAddress = (table, uid, one, one_val) => {
        if (addressData.uid !== undefined){
            ...
            fetch(`http://${apiAdress}/address/add?one=${one}&one_val=${one_val}&uid_client=${projectData.uid_client}`)
            .then(response => response.json())
            .then(response => {
    ...
    const getServiceQuestions = (uid_service) => {
        ...
        fetch(`http://${apiAdress}/service_questions?uid_service=${uid_service}`)
        .then(response => response.json())
    ...
    const getGeneralServiceQuestions = () => {
        ...
        fetch(`http://${apiAdress}/service_questions?uid_service=161`)
        .then(response => response.json())
    ...
    const activateProject = () => {
        if(projectData.status !== "active"){
            props.loadingSpinner(true)
            //validate
            fetch(`http://${apiAdress}/projects/activate?uid=${projectData.uid}&employee=${projectData.employee}`)
            .then(() => {
                addHistory("8", "")
    ...
    const cancelProject = () => {
        if(projectData.status !== "cancelled-before-qualification" && projectData.status !== "cancelled-after-qualification"){
            props.loadingSpinner(true)
            fetch(`http://${apiAdress}/projects/cancel?uid=${projectData.uid}&status=${projectData.status}&uid_cancel_reason=${(uidCancelReason === '')?"0":uidCancelReason}
    ...
    const addHistory = (uid_msg, comments) => {
        fetch(`http://${apiAdress}/client_history/add?uid_msg=${uid_msg}&uid_client=${projectData.uid_client}&uid_project=${projectData.uid}
    ...
    const getPrice = (uid_subservice) => {
        fetch(`http://${apiAdress}/subservices/get?uid=${uid_subservice}`)
        .then(response => response.json())
        .then(response => {
    ...
    const callBackLater = () => {
        ...
        if(callBackLaterData === {}){
            props.loadingSpinner(true)
            fetch(`http://${apiAdress}/callbacklater/add?uid_project=${projectData.uid}&uid_client=${projectData.uid_client}&call_back_date=${call_back_date}&followup_agent=${username}&comments=${comment_callbacklater}`)
            .then(() => {
                addHistory("3", "")
            ...
            props.loadingSpinner(true)
            fetch(`http://${apiAdress}/callbacklater/update?uid_project=${projectData.uid}&uid_client=${projectData.uid_client}&call_back_date=${call_back_date}&followup_agent=${username}&comments=${comment_callbacklater}`)
            .then(() => {
                addHistory("3", "")
                setPopupCallBackLater(false)
            })
    ...
    const flagForReview = () => {
        if(projectData.flag_for_review !== "1"){
            props.loadingSpinner(true)
            fetch(`http://${apiAdress}/flagforreview/update?uid_project=${projectData.uid}&followup_agent=${username}`)
            .then(() => {
                //addHistory("3", "")
    ...
    const duplicateProject = () => {
        props.loadingSpinner(true)
        fetch(`http://${apiAdress}/projects/duplicate?uid=${projectData.uid}`)
        .then(() => {
    ...
    const sendQuestionsEmail = (name, uid_questions) => {
        props.loadingSpinner(true)
        fetch(`http://${apiAdress}/nodemailer/sendquestions?uid_project=${projectData.uid}&uid_client=${projectData.uid_client}&uid_questions=${uid_questions}&uid_service=${projectData.uid_service}&message=${messageSendQuestions}&name=${name}`)
        .then(() => {
          addHistory("2", "");
    ...
    const getDialerPrefix = (phone) => {
        if (phone !== "" && phone !== undefined && phone !== "null" && phone !== " " && phone !== null && phone[3] !== undefined){
            ...
            fetch(`http://${apiAdress}/get_dialer_prefix?first_digits=${_phone}`)
            .then(response => response.json())
            .then(response => {
                if (response.data[0] !== undefined){
    ...
    const onClickUploadFile = (event) => {
        ...
        axios.post(`http://${apiAdress}/upload-file`, data)
        .then(res => {
        ...
        retrieveImageFromClipboard(e, function(imageBlob){
            ...
            if(imageBlob){
                ...
                img.onload = function(){
                    ...
                    axios.post(`http://${apiAdress}/upload-file`, formData)
                    .then(res => {
                        let path = res.data.data.path
                        path = path.replace("./sn_uploads/", "/sn_uploads/")
                        let content = `<p><img name="added_img_${projectData.uid}" src="https://soumissionrenovation.ca${path}" width="375" /></p>`
    ...
    const saveAddress = (terms) => {
        props.loadingSpinner(true)
        fetch(`http://${apiAdress}/address/save?street_no=${(terms[0].value)?terms[0].value:""}&street=${(terms[1].value)?terms[1].value:""}&city=${(terms[2].value)?terms[2].value:""}&province=${(terms[3].value)?terms[3].value:""}&country=${(terms[4].value)?terms[4].value:""}&uid_client=${projectData.uid_client}&phone1=${(addressData.phone1)?addressData.phone1:""}&phone2=${(addressData.phone2)?addressData.phone2:""}`)
        .then(response => response.json())
        .then(response => {
            setProjectData({...projectData, uid_address: response.data.uid_address})
            saveAjax("sr_project", projectData.uid, "uid_address", response.data.uid_address)
    ...
    const getUidCity = (name, uid_address) => {
        fetch(`http://${apiAdress}/city/get?name=${name}`)
        .then(response => response.json())
        .then(response => {
    ...
    const getZipCode = (uid_address, terms) => {
        fetch(`http://${apiAdress}/get_zipcode?address=${terms[0].value + " " + terms[1].value + " " + terms[2].value + " " + terms[3].value + " " + terms[4].value}`)
        .then(response => response.json())
        .then(response => {
